...
inner join @cdmDatabaseSchema.concept_ancestor ca on
ca.descendant_concept_id = de.drug_concept_id
inner join @cdmDatabaseSchema.concept c on
c.concept_id = ca.ancestor_concept_id
    where c.concept_id in (
          ...
          where ancestor_concept_id in (@drugClassificationIdInput)
)
...
from CTE_second cs
inner join (select distinct person_id, row_number()over(order by person_id) rn
from (SELECT distinct person_id FROM CTE_second) cs) c2 on c2.person_id = cs.person_id
);
