...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (35918581,35918692,35918491,35918290,35918559,35918527)

) I
) C UNION ALL 
SELECT 8 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (35919220,35919375,35919480,35919430,35919431,35919269)
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (35918335)
...
WHERE C.value_as_concept_id in (35919824,35919827,35919263,35919747,35919151,35919820)
...
WHERE C.value_as_concept_id in (35919664,35919321,35919223,35919795,35919415,35919132)
-- End Measurement Criteria
...
WHERE C.value_as_concept_id in (35941752,35940357,35941957,35941492,35941928,914009)
-- End Measurement Criteria

) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.END_DATE >= P.OP_START_DATE AND A.END_DATE <= DATEADD(day,0,P.END_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
...
WHERE C.value_as_concept_id in (35939324,35938150)
-- End Measurement Criteria
