...
## 3. Content of the query results
...
custom_colors61 <- c("Great" = "white", "Concept 0 or NULL" = "#336B91", "2-Billionaire" = "#FBC511")
...
## 8. Findings in mapping of source concept

```{r, echo=FALSE, results='asis'}

if ( nrow(params$plot_08_data) > 0) {
     filtered <- params$plot_08_data %>% filter(critique %in% c("Wrong mapping", "Needs mapping"))
     if (any(filtered$`record_%` > 0, na.rm = TRUE)) {
       cat("When checking the concepts in the source concept fields, the following anomalies were detected:")
     } else {
...
if (is.data.frame(params$plot_08_data) && nrow(params$plot_08_data) > 0) {
  ...
  critiques <- c("Wrong mapping", "Needs mapping")
...
# Define custom colors
custom_colors8 <- c("Great" = "white", "Needs mapping" = "#336B91", "Wrong mapping" = "#FBC511")
...
if (nrow(params$plot_08_data) > 0) {
  ...
  coord_polar(theta = "y", start=0, direction=1) + 
  geom_label_repel(
    aes(label = ifelse(critique %in% c("Needs mapping", "Wrong mapping") & `record_%` > 0, 
                       paste0(round(10000 * `record_%`, digits = 2) / 100, "%"), 
                       "")), 
    size = 3, color = "black", fill = "white", show.legend = FALSE,
    force = 25,  
...
## 9. Findings in standard concepts
...
### 10.1. Issues with Source (Other issues)

```{r , echo=FALSE, results='asis'}
if (!is.null(params$issues_source_other) && nrow(params$issues_source_other) > 0) {
  
  # Render table in report
  knitr::kable(params$issues_source_other, caption = "Issues with Source (Other)")
}else {
...
### 10.2. Issues with Source (Mapping issues)

```{r , echo=FALSE, results='asis'}
if (!is.null(params$issues_source_mapping) && nrow(params$issues_source_mapping) > 0) {
  
  # Render table in report
  knitr::kable(params$issues_source_mapping, caption = "Issues with Source (Mapping issues)")
} else {
...
### 10.3. Issues with Standard

```{r , echo=FALSE, results='asis'}
if (!is.null(params$issues_standard) && nrow(params$issues_standard) > 0) {
  
  # Render table in report
  knitr::kable(params$issues_standard, caption = "Issues with Standard")
}else {
