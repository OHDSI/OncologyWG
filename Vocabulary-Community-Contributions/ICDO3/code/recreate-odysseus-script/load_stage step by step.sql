...
DROP TABLE IF EXISTS icdo3.concept_stage;
CREATE TABLE icdo3.concept_stage (
	concept_id int4,
...
DROP TABLE IF EXISTS icdo3.concept_relationship_stage;
CREATE TABLE icdo3.concept_relationship_stage (
	concept_id_1 int4,
	concept_id_2 int4,
...
DROP TABLE IF EXISTS icdo3.concept_synonym_stage;
CREATE TABLE icdo3.concept_synonym_stage (
...
DROP TABLE IF EXISTS icdo3.pack_content_stage;
CREATE TABLE icdo3.pack_content_stage (
...
DROP TABLE IF EXISTS icdo3.drug_strength_stage;
CREATE TABLE icdo3.drug_strength_stage (
	drug_concept_code VARCHAR (20) NOT NULL,
	vocabulary_id_1 VARCHAR (20) NOT NULL,
...
-- Load input files:
-- MOVED TO SEPARATE FILE "load source files.sql"
-- r_to_c_all
...
-- Line 31 - 33
-- 2. Initial cleanup
TRUNCATE TABLE icdo3.concept_stage, icdo3.concept_relationship_stage, icdo3.concept_synonym_stage; -- icdo3.drug_strength_stage, icdo3.pack_content_stage;
...
DROP TABLE IF EXISTS icdo3.active_concept;
CREATE TABLE icdo3.active_concept AS (
    SELECT DISTINCT
      c.id,
...
DROP TABLE IF EXISTS icdo3.active_status;
CREATE TABLE icdo3.active_status AS (
    SELECT DISTINCT
      r.sourceid,
      r.destinationid,
    ...
    JOIN icdo3.active_concept AS a1
      ON a1.id = r.sourceid AND a1.active = 1
    JOIN icdo3.active_concept AS a2
...
DROP TABLE IF EXISTS icdo3.concepts;
CREATE TABLE icdo3.concepts AS (
	SELECT
		destinationid AS ancestor_concept_code,
		sourceid AS descendant_concept_code
	FROM icdo3.active_status
	WHERE active = 1
);
-- Step 4: takes ~5 minutes -> rewrite possible (doesn't seem so)
DROP TABLE IF EXISTS icdo3.hierarchy_concepts;
CREATE TABLE icdo3.hierarchy_concepts AS (
...
-- Step 5: Results in table icdo3.snomed_ancestor with 10,719,149 rows
DROP TABLE IF EXISTS icdo3.snomed_ancestor;
CREATE TABLE icdo3.snomed_ancestor AS (
	SELECT DISTINCT
		descendant_concept_code AS descendant_concept_code,
		root_ancestor_concept_code AS ancestor_concept_code 
	FROM icdo3.hierarchy_concepts
);
-- Clean up
DROP TABLE IF EXISTS icdo3.active_status, icdo3.concepts, icdo3.hierarchy_concepts;
-- Line 104-110: Results in table icdo3.snomed_ancestor with 11,510,673 rows (added 791,524 rows)
--3.2. Add relation to self for each target
INSERT INTO icdo3.snomed_ancestor
SELECT DISTINCT
  descendant_concept_code AS descendant_concept_code,
  descendant_concept_code AS ancestor_concept_code 
FROM icdo3.snomed_ancestor hc;
...
SELECT a.active, COUNT(*)
FROM icdo3.snomed_ancestor s
JOIN icdo3.active_concept a
ON a.id = s.descendant_concept_code
GROUP BY a.active;
-- !!! 643 have an inactive ancestor because 1240414004 is not active anymore (since 30112022)
-- Then:
ALTER TABLE icdo3.snomed_ancestor ADD CONSTRAINT xpksnomed_ancestor PRIMARY KEY (ancestor_concept_code,descendant_concept_code);
CREATE INDEX snomed_ancestor_d on icdo3.snomed_ancestor (descendant_concept_code);
ANALYZE icdo3.snomed_ancestor;

-- PART 4: 393-411, <1s

-- Line 144-176: creates the histology mapping between ICDO3 and SNOMED
-- Results in table icdo3.snomed_mapping with 1633 rows 
--4. Prepare updates for histology mapping from SNOMED refset
DROP TABLE IF EXISTS icdo3.snomed_mapping;
CREATE TABLE icdo3.snomed_mapping AS
SELECT DISTINCT
  referencedcomponentid as snomed_code,
  maptarget AS icdo_code
FROM snomed.der2_srefset_simplemapfull_int AS smr
JOIN icdo3.active_concept AS ac
...
-- Deletes 418 entries. Results in table icdo3.snomed_mapping with 1236 rows.
DELETE FROM icdo3.snomed_mapping m1
WHERE EXISTS
(
  SELECT
  FROM icdo3.snomed_mapping m2
  JOIN icdo3.snomed_ancestor a 
...
GROUP BY icdo_code
ORDER BY icdo_code;
-- Deletes 186 entries (78 ICDO3 codes). Results in table icdo3.snomed_mapping with 1029 rows.
DELETE FROM icdo3.snomed_mapping
WHERE icdo_code IN
(
  SELECT icdo_code
  FROM icdo3.snomed_mapping
...
SET
  ...
  (
    SELECT 
	  s.snomed_code
    FROM icdo3.snomed_mapping s
    WHERE r.concept_code = s.icdo_code
  )
WHERE r.concept_code IN (SELECT s.icdo_code FROM icdo3.snomed_mapping s) AND r.precedence IS NULL; -- no automated modification for concepts with alternating mappings
...
DROP TABLE IF EXISTS icdo3.concept_stage CASCADE;
CREATE TABLE icdo3.concept_stage(
  CONCEPT_ID INTEGER,
  CONCEPT_NAME VARCHAR(255) NOT NULL,
...
TRUNCATE TABLE icdo3.concept_stage;

-- PART 10: 565-749, <1s

--10. Populate_concept stage with attributes
--10.1. Topography
INSERT INTO icdo3.concept_stage (
  CONCEPT_ID,
  CONCEPT_NAME,
  DOMAIN_ID,
  VOCABULARY_ID,
...
WHERE m.concept_code LIKE '%/%' AND m.concept_code NOT IN (SELECT concept_code FROM icdo3.concept_stage WHERE concept_class_id = 'ICDO Histology');
...
JOIN omopcdm.vocabulary v ON v.vocabulary_id = cm.vocabulary_id
-- No latest_update in vocabulary
WHERE v.latest_update IS NOT NULL AND NOT EXISTS (SELECT 1 FROM icdo3.concept_stage cs_int WHERE cs_int.concept_code = cm.concept_code AND cs_int.vocabulary_id = cm.vocabulary_id);
--WHERE NOT EXISTS (SELECT 1 FROM icdo3.concept_stage cs_int WHERE cs_int.concept_code = cm.concept_code AND cs_int.vocabulary_id = cm.vocabulary_id);
...
DROP TABLE IF EXISTS icdo3.code_replace;
-- Line 371-388:
...
CREATE TABLE icdo3.code_replace AS
SELECT DISTINCT
  code AS old_code,
...
INSERT INTO icdo3.code_replace
SELECT DISTINCT
  d2.concept_code AS old_code,
  d1.concept_code AS code,
  'ICDO Histology' AS concept_class_id
FROM icdo3.concept_stage d1
JOIN icdo3.concept_stage d2 
...
INSERT INTO icdo3.comb_table
SELECT
  r.code,
  c.site,
  r.code || '-' || c.site AS concept_code
FROM icdo3.comb_table c
JOIN icdo3.code_replace r 
  ON r.old_code = c.histology_behavior
WHERE (r.code,c.site,r.code || '-' || c.site) NOT IN (SELECT * FROM icdo3.comb_table);
INSERT INTO icdo3.code_replace
SELECT
...
INSERT INTO icdo3.concept_stage (CONCEPT_NAME,DOMAIN_ID,VOCABULARY_ID,CONCEPT_CLASS_ID,STANDARD_CONCEPT,CONCEPT_CODE,VALID_START_DATE,VALID_END_DATE,INVALID_REASON)
SELECT DISTINCT
  ...
  END
FROM icdo3.comb_table c
JOIN icdo3.concept_stage m 
  ON c.histology_behavior = m.concept_code
JOIN icdo3.concept_stage t 
 ON	c.site = t.concept_code
LEFT JOIN icdo3.code_replace r 
...
INSERT INTO icdo3.concept_stage (CONCEPT_ID,CONCEPT_NAME,DOMAIN_ID,VOCABULARY_ID,CONCEPT_CLASS_ID,STANDARD_CONCEPT,CONCEPT_CODE,VALID_START_DATE,VALID_END_DATE,INVALID_REASON)
SELECT
  NULL,
...
DROP TABLE IF EXISTS icdo3.snomed_target_prepared;
-- Step 1:
DROP TABLE IF EXISTS icdo3.def_status;
CREATE TABLE icdo3.def_status AS --form list of defined neoplasia concepts without extraneous relations
(
  SELECT DISTINCT 
    c.concept_code,
...
-- Step 2:
DROP TABLE IF EXISTS icdo3.snomed_concept;
CREATE TABLE icdo3.snomed_concept AS
(
  SELECT
    c.concept_id,
    c.concept_code,
  ...
  LEFT JOIN omopcdm.concept_relationship r1  --refers to morphologies that are not neoplasms
    ...
    (
      SELECT
      FROM icdo3.snomed_ancestor AS sa
...
DROP TABLE icdo3.tmp1;
DROP TABLE icdo3.tmp2;
CREATE INDEX idx_snomed_target_prepared ON icdo3.snomed_target_prepared (concept_code);
CREATE INDEX idx_snomed_target_attr ON icdo3.snomed_target_prepared (m_id, t_id);
CREATE INDEX idx_snomed_target_t ON icdo3.snomed_target_prepared (t_id);
ANALYZE icdo3.snomed_target_prepared;
DELETE FROM icdo3.snomed_target_prepared a
WHERE a.t_id = '-1' 
  AND EXISTS
  (
    SELECT
    FROM icdo3.snomed_target_prepared b
    WHERE a.concept_code = b.concept_code AND b.t_id != '-1'
  );
ANALYZE icdo3.snomed_target_prepared;

-- PART 14: 1109-1303, ~10s

-- Line 701-745:
--14. Form mass of all possible matches to filter later
DROP TABLE IF EXISTS icdo3.match_blob;
CREATE TABLE icdo3.match_blob AS
SELECT DISTINCT
...
WHERE o.concept_code NOT IN (SELECT old_code FROM icdo3.code_replace);
CREATE INDEX idx_blob ON icdo3.match_blob (i_code, s_id);
CREATE INDEX idx_blob_s ON icdo3.match_blob (s_id);
ANALYZE icdo3.match_blob;
-- Line 786-838:
...
DELETE FROM icdo3.match_blob m
WHERE NOT m.t_exact -- for lymphomas/leukemias
  AND EXISTS
  (
    SELECT 1
    FROM icdo3.snomed_target_prepared r
...
DELETE FROM icdo3.match_blob
WHERE s_id IN
  (
    SELECT descendant_concept_code::text
    FROM icdo3.snomed_ancestor 
...
DROP TABLE icdo3.tmp1;
--debug artifact
TRUNCATE TABLE icdo3.concept_relationship_stage;

-- PART 16: 1455-2097, ~1s
...
-- Step 1:
DROP TABLE IF EXISTS icdo3.monorelation;
CREATE TABLE icdo3.monorelation AS
(
  SELECT i_code
  FROM icdo3.match_blob
  WHERE t_exact AND m_exact
  GROUP BY i_code
  HAVING COUNT (DISTINCT s_id) = 1
);
-- Step 2:
INSERT INTO icdo3.concept_relationship_stage (concept_code_1,concept_code_2,vocabulary_id_1,vocabulary_id_2,relationship_id,valid_start_date,valid_end_date)
SELECT DISTINCT
  m.i_code,
...
DROP TABLE IF EXISTS icdo3.icdo3_to_cm_metastasis;
--ICDO3 /6 codes mappings to Cancer Modifier
-- Step 1:
DROP TABLE IF EXISTS icdo3.getherd_mts_codes;
CREATE TABLE icdo3.getherd_mts_codes AS
(
...
-- Step 2:
DROP TABLE IF EXISTS icdo3.tabb;
CREATE TABLE icdo3.tabb AS
(
  SELECT DISTINCT
    tumor_site_code,
    s.vocabulary_id,
  ...
  FROM icdo3.getherd_mts_codes s
  LEFT JOIN omopcdm.concept c
...
-- Step 3:
DROP TABLE IF EXISTS icdo3.tabbc;
CREATE TABLE icdo3.tabbc AS
(
  SELECT 
    tumor_site_code,
    ...
    c.valid_end_date,
    c.invalid_reason
  FROM icdo3.tabb t -- table with SITEtoSNOMED mappngs
...
DROP TABLE IF EXISTS icdo3.similarity_tab;
CREATE TABLE icdo3.similarity_tab AS
(
  SELECT DISTINCT
    CASE 
...
-- Step 5:
CREATE TABLE icdo3.icdo3_to_cm_metastasis AS
(
  SELECT DISTINCT
    ...
    s.concept_name,
    s.vocabulary_id
  FROM icdo3.similarity_tab s
  JOIN icdo3.getherd_mts_codes a
    ON s.tumor_site_code = a.tumor_site_code
  WHERE similarity=1
);
-- Line 1108-1133:
--Assumption MTS
INSERT INTO icdo3.icdo3_to_cm_metastasis (icd_name, icd_code, tumor_site_code, icd_vocab, concept_id, concept_code, concept_name, vocabulary_id)
SELECT DISTINCT
  ...
  m.concept_name,
  m.vocabulary_id
FROM icdo3.concept_stage s
JOIN icdo3.icdo3_to_cm_metastasis m
  ON SPLIT_PART(SPLIT_PART(s.concept_code,'-',2),'.',1)||'.9' = m.tumor_site_code
WHERE s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) AND s.concept_code LIKE '%/6-%';
-- Line 1134-1159:
...
SELECT DISTINCT 
  ...
  c.concept_name,
  c.vocabulary_id
FROM icdo3.concept_stage s, omopcdm.concept  c
WHERE c.concept_code = 'OMOP4998770' AND c.vocabulary_id ='Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) 
...
SELECT DISTINCT 
  ...
  c.vocabulary_id
FROM icdo3.concept_stage s, omopcdm.concept  c
WHERE c.concept_code = 'OMOP4999341' AND c.vocabulary_id ='Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)
...
(
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031980' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) 
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031483' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Metastasis to the Anal Canal
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031707' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031839' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)--	Metastasis to the Retroperitoneum And Peritoneum'
  ...
  SELECT DISTINCT
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031916' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Metastasis to the Soft Tissues
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031618' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)--	Metastasis to the Female Genital Organ
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031819' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Metastasis to the Prostate
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5031716' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Metastasis to the Male Genital Organ
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5117515' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Metastasis to meninges NEW CONCEPT
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5117516' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)  --	Metastasis to abdomen --new concept
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP4998263' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --Lymph Nodes
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP4998263' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --Lymph Nodes -- TODO NEW CODE NEEDED (not sure that /6 resembles always distant)
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP5000384' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis)  --	Inguinal Lymph Nodes
  ...
  SELECT DISTINCT 
    ...
    c.vocabulary_id
  FROM icdo3.concept_stage s, omopcdm.concept c
  WHERE c.concept_code = 'OMOP4999638' AND c.vocabulary_id = 'Cancer Modifier' AND s.concept_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis) --	Pelvic Lymph Nodes
...
WHERE icd_code NOT IN (SELECT icd_code FROM icdo3.icdo3_to_cm_metastasis);
UPDATE icdo3.icdo3_to_cm_metastasis SET icd_vocab ='ICDO3' WHERE icd_vocab !='ICDO3';
-- Line 1531-1551:
...
FROM icdo3.icdo3_to_cm_metastasis;
-- Line 1552-1570:
-- Add this for later
ALTER TABLE icdo3.concept_relationship_stage ADD CONSTRAINT idx_pk_crs PRIMARY KEY (concept_code_1,concept_code_2,vocabulary_id_1,vocabulary_id_2,relationship_id);
...
BEGIN
  ...
  JOIN icdo3.concept_relationship_stage s ON s.concept_code_1 = m.concept_code_1 AND s.vocabulary_id_1 = m.vocabulary_id_1 AND m.invalid_reason IS NULL AND m.relationship_id = 'Maps to';
...
BEGIN
	SELECT s0.reason INTO z FROM (
		...
		FROM sources.concept_relationship_manual crm
			...
			LEFT JOIN icdo3.concept_stage cs1 ON cs1.concept_code = crm.concept_code_1 AND cs1.vocabulary_id = crm.vocabulary_id_1
			LEFT JOIN omopcdm.concept c2 ON c2.concept_code = crm.concept_code_2 AND c2.vocabulary_id = crm.vocabulary_id_2
			LEFT JOIN icdo3.concept_stage cs2 ON cs2.concept_code = crm.concept_code_2 AND cs2.vocabulary_id = crm.vocabulary_id_2
...
--	END IF;
	...
	INSERT INTO icdo3.concept_relationship_stage AS crs (
		concept_code_1,
		concept_code_2,
		vocabulary_id_1,
		vocabulary_id_2,
...
SELECT DISTINCT
  ...
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.match_blob m
LEFT JOIN icdo3.concept_relationship_stage r 
  ON m.i_code = r.concept_code_1
  AND r.relationship_id = 'Maps to'
WHERE r.concept_code_1 IS NULL;

-- PART 17: 2098-2135, <1s
...
WHERE concept_code NOT IN
  (
    ...
    FROM icdo3.concept_relationship_stage
  ) 
  AND concept_code != '9999/9' 
  AND snomed_code != '-1';
  
-- PART 18: 2136-2200, <1s
  
-- Line 1631-1633:
--18. Create internal hierarchy for attributes and combos
DROP TABLE IF EXISTS icdo3.attribute_hierarchy;
-- Line 1634-1691: In steps
-- Step 1:
CREATE TABLE icdo3.hierarchy AS
(
  SELECT
    level, -- 2 and 3
  ...
  FROM sources.morph_source_who 
  JOIN icdo3.concept_stage 
    ON concept_code = icdo32
  WHERE level IN ('2','3')
);
-- Step 2:
CREATE TABLE icdo3.relation_hierarchy AS
...
(
  ...
  FROM icdo3.hierarchy h2
  JOIN icdo3.hierarchy h3 
...
-- Step 3:
CREATE TABLE icdo3.relation_atom AS
(
  SELECT
    ...
    'A' AS reltype --Atom			
  FROM icdo3.concept_stage s
  JOIN icdo3.hierarchy h 
    ON s.concept_code BETWEEN h.start_code AND h.end_code || 'Z' -- to guarantee inclusion of upper bound
  WHERE s.concept_class_id = 'ICDO Histology' AND s.concept_code NOT IN (SELECT old_code FROM icdo3.code_replace) AND s.standard_concept IS NULL 
    --avoid jump from 2 to atom where 3 is available; concept_ancestor will not care
	AND h.concept_code NOT IN (SELECT ancestor_code FROM icdo3.relation_hierarchy)
    --obviously excludde hierarchical concepts
    AND s.concept_code NOT IN (SELECT concept_code FROM icdo3.hierarchy)
);
-- Step 4:
CREATE TABLE icdo3.attribute_hierarchy AS
SELECT *
FROM icdo3.relation_hierarchy
  UNION ALL
SELECT *
FROM icdo3.relation_atom;
-- Line 1692-1711:
--18.2. Internal hierarchy for topography attribute
INSERT INTO icdo3.attribute_hierarchy
SELECT
  t1.code,
  t2.code,
  'A'
...
SELECT DISTINCT
  ...
  TO_DATE ('19700101', 'yyyymmdd'),
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.attribute_hierarchy a
WHERE a.ancestor_code != a.descendant_code;

-- PART 20: 2217-2347, ~30s
-- If this takes longer, run until 2337 and run second part afterwards
...
SELECT DISTINCT
  ...
  TO_DATE ('19700101', 'yyyymmdd'),
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.comb_table c1
LEFT JOIN icdo3.code_replace c2 
...
SELECT DISTINCT
  ...
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.concept_stage s
JOIN icdo3.concept_relationship_stage r 
...
SELECT
  ...
  TO_DATE ('19700101', 'yyyymmdd'),
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.comb_table s
LEFT JOIN icdo3.concept_relationship_stage x  
...
WHERE x.concept_code_1 IS NULL AND r1.snomed_code != '-1' AND NOT EXISTS (SELECT 1 FROM icdo3.concept_relationship_stage a WHERE a.concept_code_1 = s.concept_code AND a.concept_code_2 = r1.snomed_code::text)
  UNION ALL
---Histology
SELECT
  s.concept_code,
...
--DELETE FROM icdo3.concept_relationship_stage s
--WHERE relationship_id IN ('Has asso morph','Has finding site')
--AND EXISTS
--(
--  SELECT
--  FROM icdo3.concept_relationship_stage s2
...
SELECT DISTINCT
	...
	TO_DATE ('19700101', 'yyyymmdd'),
	TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.code_replace cr;

-- PART 22: 2364-2372, <1s

-- Line 1881-1897:
--22. Make concepts without 'Maps to' relations Standard
UPDATE icdo3.concept_stage
SET standard_concept = 'S'
WHERE invalid_reason IS NULL AND concept_code NOT IN (SELECT concept_code_1 from icdo3.concept_relationship_stage WHERE relationship_id = 'Maps to') 
...
INSERT INTO icdo3.concept_synonym_stage(
    synonym_name,
...
BEGIN
	...
	DELETE
	FROM icdo3.concept_relationship_stage crs
	WHERE EXISTS (
			SELECT 1
			FROM icdo3.concept_relationship_stage cs1,
				icdo3.concept_relationship_stage cs2
...
--	SET valid_end_date = cs.valid_end_date,
	...
	AS (
		WITH RECURSIVE rec AS (
			...
			upgraded_concepts AS (
				...
				FROM icdo3.concept_relationship_stage crs
				LEFT JOIN icdo3.concept_stage cs ON crs.concept_code_2 = cs.concept_code
	...
	UPDATE icdo3.concept_relationship_stage crs
...
SELECT
  ...
  TO_DATE ('19700101', 'yyyymmdd'),
  TO_DATE ('20991231', 'yyyymmdd')
FROM icdo3.concept_relationship_stage r
LEFT JOIN icdo3.concept_relationship_stage r2 
ON r2.relationship_id = 'Maps to' AND r.concept_code_2 = r2.concept_code_1
WHERE r.relationship_id = 'Concept replaced by' AND NOT EXISTS (SELECT 1 FROM icdo3.concept_relationship_stage r3 WHERE r.concept_code_1 = r3.concept_code_1 AND r3.relationship_id = 'Maps to');
-- Line 1951-1956:
...
$BODY$
	...
	*/
BEGIN
	ANALYZE icdo3.concept_relationship_stage, icdo3.concept_stage;
	...
	CREATE TEMP TABLE has_rel_with_comp ON COMMIT DROP AS
		...
		FROM icdo3.concept_relationship_stage crs_int1
		JOIN icdo3.concept_relationship_stage crs_int2 ON crs_int2.concept_code_2 = crs_int1.concept_code_2
		...
		JOIN icdo3.concept_stage cs_int ON cs_int.concept_code = crs_int1.concept_code_2
...
-- PART 26: 2857-2864, <1s

-- Line 1963-1977:
--26. If concept got replaced, give it invalid_reason = 'U'
UPDATE icdo3.concept_stage x
SET invalid_reason = 'U'
WHERE invalid_reason = 'D' AND EXISTS (SELECT 1 FROM icdo3.concept_relationship_stage WHERE invalid_reason IS NULL AND relationship_id = 'Concept replaced by' AND concept_code_1 = x.concept_code);

-- PART 27: 2865-2873, <1s
...
INSERT INTO icdo3.concept_relationship_stage (concept_code_1,concept_code_2,vocabulary_id_1,vocabulary_id_2,relationship_id,valid_start_date,valid_end_date, invalid_reason)
SELECT
  c.concept_code,
  c2.concept_code,
  'ICDO3',
...
LEFT JOIN icdo3.concept_relationship_stage s 
...
LEFT JOIN icdo3.concept_relationship_stage s 
  ON s.concept_code_1 = c.concept_code AND s.concept_code_2 = c2.concept_code AND s.relationship_id = r.relationship_id
WHERE s.concept_code_1 IS NULL AND NOT (c.concept_id = c2.concept_id AND r.relationship_id = 'Maps to' AND EXISTS (SELECT 1 FROM icdo3.concept_stage x WHERE x.concept_code = c.concept_code));
--don't deprecate maps to self for Standard concepts

-- PART 30: 2963-2970, <1s

-- Line 2093-2095:
-- 30. Cleanup: drop all temporary tables
DROP TABLE IF EXISTS icdo3.snomed_mapping, icdo3.snomed_target_prepared, icdo3.attribute_hierarchy, icdo3.comb_table, icdo3.match_blob, icdo3.code_replace, icdo3.snomed_ancestor, icdo3.icdo3_to_cm_metastasis;
-- And our own intermediate tables
DROP TABLE IF EXISTS icdo3.active_concept, icdo3.def_status, icdo3.getherd_mts_codes, icdo3.hierarchy, icdo3.monorelation;
DROP TABLE IF EXISTS icdo3.relation_atom, icdo3.relation_hierarchy, icdo3.similarity_tab, icdo3.snomed_concept, icdo3.tabb, icdo3.tabbc;