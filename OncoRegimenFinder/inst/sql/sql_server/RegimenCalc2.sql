IF OBJECT_ID('@writeDatabaseSchema.@regimenTable_tmp', 'U') IS NOT NULL
DROP TABLE @writeDatabaseSchema.@regimenTable_tmp;
...
WITH add_groups AS (
  ...
  LEFT JOIN @writeDatabaseSchema.@regimenTable r2 on r1.person_id = r2.person_id and
                                                     ...
                                                     r2.ingredient_start_date >= DATEADD(DAY, -@date_lag_input, r1.ingredient_start_date)
...
IF OBJECT_ID('@writeDatabaseSchema.@regimenTable', 'U') IS NOT NULL
DROP TABLE @writeDatabaseSchema.@regimenTable;
