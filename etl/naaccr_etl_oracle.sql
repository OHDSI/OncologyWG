...
BEGIN
  ...
  EXECUTE IMMEDIATE 'DROP TABLE episode_temp';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
...
CREATE TABLE procedure_occurrence_temp
 ...
 BEGIN
  ...
  EXECUTE IMMEDIATE 'DROP TABLE drug_exposure_temp';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
...
-- Treatment Episodes
  ...
  INNER JOIN concept c1 ON c1.concept_class_id = 'NAACCR Variable' AND ndp.naaccr_item_number = c1.concept_code
	INNER JOIN concept_relationship cr1 ON c1.concept_id = cr1.concept_id_1 AND cr1.relationship_id = 'Has Answer'
	INNER JOIN concept c2 ON cr1.concept_id_2 = c2.concept_id AND CONCAT(c1.concept_code, CONCAT('@', ndp.naaccr_item_value)) = c2.concept_code AND c2.domain_id = 'Procedure'
...
-- Update episode_event_temp
  ...
  --Step 17: Observation
  BEGIN
  EXECUTE IMMEDIATE 'TRUNCATE TABLE observation_temp';
  EXECUTE IMMEDIATE 'DROP TABLE observation_temp';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
...
END;
  ...
  --	  , NULL																																					                                                                                     AS value_as_datetime
        , ndp.record_id                                                                                                                                                      AS record_id
  FROM naaccr_data_points_temp  ndp INNER JOIN concept d                             ON d.vocabulary_id = 'NAACCR' AND d.concept_code = ndp.naaccr_item_number ||  '@' || ndp.naaccr_item_value
                                      INNER JOIN concept_relationship cr1              ON d.concept_id = cr1.concept_id_1 AND cr1.relationship_id = 'Maps to'
                                      INNER JOIN concept  c1                         ON cr1.concept_id_2 = c1.concept_id AND c1.vocabulary_id = 'NAACCR' AND c1.concept_class_id = 'NAACCR Value' AND c1.domain_id = 'Observation' AND c1.standard_concept = 'S'
  ...
  BEGIN
  EXECUTE IMMEDIATE 'TRUNCATE TABLE fact_relationship_temp';
  EXECUTE IMMEDIATE 'DROP TABLE fact_relationship_temp';
EXCEPTION
  WHEN OTHERS THEN
    IF SQLCODE != -942 THEN
      RAISE;
...
END;
  ...
  SELECT 32527                             AS domain_concept_id_1			  -- Episode
