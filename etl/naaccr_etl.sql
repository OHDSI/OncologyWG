...
DROP TABLE IF EXISTS condition_occurrence_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS measurement_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS episode_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS episode_event_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS procedure_occurrence_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS drug_exposure_temp;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS ambig_schema_discrim;           -- Drop temp table if it exists
...
DROP TABLE IF EXISTS naaccr_data_points_tmp;           -- Drop temp table if it exists
...
-- DATA PREP
  ...
  AND LENGTH(naaccr_data_points_tmp.naaccr_item_value) < 10
...
--- DIAGNOSIS
  ...
  --   condition modifiers
    ...
    SELECT ( CASE WHEN  (SELECT MAX(measurement_id) FROM measurement) IS NULL
      ...
      END + row_number() over ()) AS measurement_id
  ...
  THEN 0 ELSE  (SELECT MAX(measurement_id) FROM measurement_temp) END + row_number() over()) AS measurement_id
...
SELECT ( CASE WHEN  (SELECT MAX(measurement_id) FROM measurement_temp) IS NULL THEN 0 ELSE
  (SELECT MAX(measurement_id) FROM measurement_temp) END + row_number() over()) AS measurement_id
      , mt.person_id                                                                                                                                                      AS person_id
      , mt.measurement_concept_id                                                                                                                                         AS measurement_concept_id
      , mt.measurement_date                                                                                                                                               AS measurement_date
      , mt.measurement_time                                                                                                                                               AS measurement_time
